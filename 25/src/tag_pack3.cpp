//$Id: tag_pack3.cpp 3125 2013-11-19 02:44:01Z jsibert $
#include <fstream>
#include "par_t_reg.h"
#include "tag_pack3.h"
#include "trace.h"
...
int tag_packet::norm_walk(int& _seed, year_month& date)
{
  ...
  while (k == 0) // keep jumping until current position is in the ocean
  {
    ...
    //ipar[83] == 1
    dmatrix sigma(1,m,1,n); 
    pptr->sigma_comp(sigma,date);
    
    dmatrix u(1,m,1,n);
    dmatrix v(1,m,1,n);
    int s = pptr->get_season(date);
    int t = date-pptr->get_start_date()+1;
    pptr->uvcomp(u,v,s,t);
    //ipar[83] == 1
    ...
    if (k > 0) 
    {
      ...
      dx = (sigma(i,j) * my_randn(seed) + edx)/60.0;
      dy = (sigma(i,j) * my_randn(seed) + edy)/60.0;*/
      ...
      // ipar[83]==1 Compute jump distances in Nmi and convert to degrees
      dx = (sqrt(sigma(i,j)/15) * my_randn(seed) + u(i,j))/60.0;
      dy = (sqrt(sigma(i,j)/15) * my_randn(seed) + v(i,j))/60.0;
...
double tag_packet::get_displacement(const trv_ptr& _trv)
{
  trv_ptr trv = (trv_ptr)_trv;
