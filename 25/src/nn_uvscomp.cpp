//$Id: nn_uvscomp.cpp 2754 2011-01-02 20:57:07Z jsibert $
#include "par_t_nn.h"
#include "trace.h"
...
void par_t_nn<D3_ARRAY,MATRIX,VECTOR,DOUBLE>::uvs_comp(MATRIX& u, MATRIX& v, MATRIX& sigma, year_month& ym)
{
  ...
  if(display_nodes(14)  )
  {
       ...
       //zonal_current *= (-1) * 3600 * 24 * 30 / 185200;
  }
  if(display_nodes(15)  )
  {
       habitat_data(meridional_current_file_ext, ym, meridional_current);
       //meridional_current *= (-1) * 3600 * 24 * 30 / 185200;
  }
  if(display_nodes(16)  )
       habitat_data(habitat_file_ext, ym, habitat);

  //iobjects for uvs_diagnose function
  MATRIX unode(1,m,1,n);  unode.initialize();
  MATRIX vnode(1,m,1,n);  vnode.initialize();
  MATRIX snode(1,m,1,n);   snode.initialize();
  ...
  MATRIX output_layer(1,num_nn,1,voutput);     output_layer.initialize();
  ...
  for (int i=1; i <= m; i++)
  {
    ...
    for (int j = j1; j <= jn; j++)
    {
      ...
      //clogf <<"nnout:  " << input_layer(2,1) << "   " << input_layer(2,2) << "  " << input_layer(2,3) << endl;
	...
	const DOUBLE& nn_output1 = nn_output(1);
	const DOUBLE& nn_output2 = nn_output(2);
	const DOUBLE& nn_output3 = nn_output(3);
        u(i,j) = scalefn(nn_output1, u_slope, u_offset, minvel, maxvel);
        v(i,j) = scalefn(nn_output2, v_slope, v_offset, minvel, maxvel);
        if(m_ipar[77] == 0 )
        {
          sigma(i,j) = scale_fn(nn_output3, sig_slope, sig_offset, minsig, maxsig);
        }
        else if(m_ipar[77] == 1)
        {
          o3 = nn_output3 * nn_output3;
          sigma(i,j) = scale_fn(o3,sig_slope, sig_offset, minsig, maxsig);
        }
        else if(m_ipar[77] == 2)
        {
          DOUBLE rs = scale_fn(nn_output3, sig_slope, sig_offset, minsig, maxsig);
          sigma(i,j) = rs * rs;
        }
        else if(m_ipar[77] == 3)
        {
     ...
     {
       int seed = m_ipar[40];
       clogf.setf(ios::scientific,ios::floatfield);
       if (ran1(&seed) < 0.1)
...
template void par_t_nn<d3_array,dmatrix,dvector,double>::uvs_comp(dmatrix& u, dmatrix& v, dmatrix& sigma, year_month& ym);
template void par_t_nn<dvar3_array,dvar_matrix,dvar_vector,dvariable>::uvs_comp(dvar_matrix& u, dvar_matrix& v, dvar_matrix& sigma, year_month& ym);
