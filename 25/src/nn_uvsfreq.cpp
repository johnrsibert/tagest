//$Id: nn_uvsfreq.cpp 2754 2011-01-02 20:57:07Z jsibert $
#include <fvar.hpp>
//#include <iomanip.h>
#include "trace.h"
...
void uvs_freq_comp(const MATRIX& unode, const MATRIX& vnode, const MATRIX& snode, const imatrix& gridmap, const int report)
{
  ...
  //TRACE(snode)
   /***************************************************************************************
   *  Purpose:
   *  To calculate the frequencies of the u,v and sigma. NOTE: values greater or equal to the last bin
   *  would be allocated to the last bin. For instance  if the bin arrangements were as follows:
   *         bin       freq
   *           0        xx
   *       100        xx
   *       200        xx
   *  Than values greater than 200 would go to the last bin. This is not what happens in  Excel's
   *  <frequency> function. In that case values greater than 200 would be excluded. To avoid this happening
   *  make sure to have enough bins so that largest value will not exceed the largest bin size. ....msa
    ***************************************************************************************/
...
template void uvs_freq_comp(const dmatrix& unode, const dmatrix& vnode, const dmatrix& snode, const imatrix& gridmap, const int report);
template<> void uvs_freq_comp(const dvar_matrix& unode, const dvar_matrix& vnode, const dvar_matrix& snode, const imatrix& gridmap, const int report)
	{ uvs_freq_comp(value(unode), value(vnode), value(snode), gridmap, report); }
