//$Id: rfgcomp2.cpp 2842 2011-08-15 22:35:14Z jsibert $
#include "par_t_reg.h"
#include "intersav.h"
#include "fish_rec.h"
#include "trace.h"
...
//void df_fish_mort_comp(par_t<double>& dfparam, d3_array& dff_mort, d3_array& effort);
...
void compute(MATRIX& release,
	     ...
	     const int nfleet,
	     DOUBLE& plike,
	     par_t_reg<D3_ARRAY,MATRIX,VECTOR,DOUBLE>& param)
{
        DOUBLE plike1 = 0.0;
        switch (param.m_ipar[11])
        {
          case 2:
            ...
            {
              cerr << "Unknown option (" << param.m_ipar[27]
...
template <typename D3_ARRAY, typename MATRIX, typename VECTOR, typename DOUBLE>
void fcomp2(const year_month &local_start_date, const year_month &final_date,
                const int pool_tags, int &kmonth, int &last_rn, const int ncohort, MATRIX& release,
		int &current_season,
                MATRIX& u, MATRIX& v, MATRIX& sigma,
coff_t<D3_ARRAY,MATRIX,VECTOR,DOUBLE> &coff, ivector &coff_comp_called, double &total_uv_penalty, DOUBLE& plike, double &total_sigma_penalty,
indexed_regional_fishery_record& irfr, d3_array &effort, ivector &effort_occured, D3_ARRAY& fish_mort, MATRIX& tot_mort,
...
par_t_reg<D3_ARRAY,MATRIX,VECTOR,DOUBLE> &param) {
    ...
    for (year_month date = local_start_date; date <= final_date; date++) {
      ...
      if (current_season != param.get_season(date))
      {
        //clogf << "Season Change Forward "; TTRACE(date,get_season(date))
        current_season = param.get_season(date);
        ...
        //added jca
        DOUBLE penlike = 0.0;
        velpen<D3_ARRAY,MATRIX,VECTOR,DOUBLE>(param, penlike, date);
...
template <typename D3_ARRAY, typename MATRIX, typename VECTOR, typename DOUBLE>
void backward(const year_month& final_date,
		ivector& coff_comp_called,
	      const int pool_tags,
	      dmatrix& dfrelease,
	      MATRIX& release,
	      ...
	      coff_t<d3_array,dmatrix,dvector,double>& dfcoff,
              par_t_reg<D3_ARRAY,MATRIX,VECTOR,DOUBLE>& param,
              par_t_reg<d3_array,dmatrix,dvector,double>& dfparam,
	      ...
	      MATRIX& tot_mort,
	      int& kmonth,
	      int& omit_release_month,
	      int& current_season,
	      int& cohort,
	      int& nfleet,
		recaptype_vector& recaps, const int nrec,
	      double& dfplike,
	      double& dftot_tag_pen,
	      double& dfneg_tag_pen,
	      const year_month& local_start_date)
{
	for (year_month date = final_date; date >= local_start_date; date--) {
      ...
      //negative_tag_penalty(release, param, neg_tag_pen, negatives);
      ...
      if(coff_comp_called(kmonth))
      {
        ...
        current_season = param.get_season(date1);

        dfsigpen(dfparam, param, date);
        dfvelpen<D3_ARRAY,MATRIX,VECTOR,DOUBLE>(dfparam, param, date);

        param.uvs_comp(u, v, sigma, date);   // recompute
        dfcoff.dfcomp(coff, param, dfparam, u, v, sigma, date,
          df_u, df_v, df_sigma, isp);
        //clogf << "               season: " << get_season(date) << endl;

        param.dfuvs_comp2(param, dfparam, df_u, df_v, df_sigma, date);
      ...
      {
        year_month date1 = date - 1;
        get_effort_array(param, irfr, effort, date1, effort_occured);
        param.fish_mort_comp(fish_mort, effort, date1);
        param.total_mort_comp(fish_mort, tot_mort);
        coff.b_adjust(param, tot_mort);
      } //end_if
...
template 
void backward<d3_array,dmatrix,dvector,double>(const year_month& final_date,
		ivector& coff_comp_called,
	      const int pool_tags,
	      dmatrix& dfrelease,
	      dmatrix& release,
	      ...
	      coff_t<d3_array,dmatrix,dvector,double>& dfcoff,
              par_t_reg<d3_array,dmatrix,dvector,double>& param,
              par_t_reg<d3_array,dmatrix,dvector,double>& dfparam,
	      indexed_regional_fishery_record& irfr, 
	      dmatrix& u,
	      dmatrix& v,
		...
		recaptype_vector& recaps, const int nrec,
	      ...
	      const year_month& local_start_date);
template <>
void backward<dvar3_array,dvar_matrix,dvar_vector,dvariable>(const year_month& final_date,
		ivector& coff_comp_called,
	      const int pool_tags,
	      dmatrix& dfrelease,
	      dvar_matrix& release,
	      ...
	      coff_t<d3_array,dmatrix,dvector,double>& dfcoff,
              par_t_reg<dvar3_array,dvar_matrix,dvar_vector,dvariable>& param,
              par_t_reg<d3_array,dmatrix,dvector,double>& dfparam,
	      indexed_regional_fishery_record& irfr, 
	      dvar_matrix& u,
	      dvar_matrix& v,
...
template <typename D3_ARRAY, typename MATRIX, typename VECTOR, typename DOUBLE>
void par_t_reg<D3_ARRAY,MATRIX,VECTOR,DOUBLE>::fgcomp2(DOUBLE& plike, VECTOR& x, dvector &g, int _nvar, recaptype_vector &recaps, int nrec, coff_t<D3_ARRAY,MATRIX,VECTOR,DOUBLE> &coff, coff_t<d3_array,dmatrix,dvector,double>& dfcoff, indexed_regional_fishery_record &irfr)
{
  int   i, j, cohort;
...
//dvariable templike = 0;
	fcomp2<D3_ARRAY,MATRIX,VECTOR,DOUBLE>(
		local_start_date, final_date,
                pool_tags, kmonth, last_rn, ncohort, release,
		current_season,
		u, v, sigma,
//coff, coff_comp_called, total_uv_penalty, templike, total_sigma_penalty,
coff, coff_comp_called, total_uv_penalty, plike, total_sigma_penalty,
irfr, effort, effort_occured, fish_mort, tot_mort,
cohort, recaps, nrec,
nfleet,
...
//cout << gradient_structure::totalbytes() << endl;
	...
	backward<D3_ARRAY,MATRIX,VECTOR,DOUBLE>(final_date,
		coff_comp_called,
	      pool_tags,
	      dfrelease,
	      release,
...
template void par_t_reg<d3_array,dmatrix,dvector,double>::fgcomp2(double& plike, dvector& x, dvector& g, int _nvar, recaptype_vector& recaps, int nrec, coff_t<d3_array,dmatrix,dvector,double> &coff, coff_t<d3_array,dmatrix,dvector,double> &dfcoff, indexed_regional_fishery_record& irfr);
template void par_t_reg<dvar3_array,dvar_matrix,dvar_vector,dvariable>::fgcomp2(dvariable& plike, dvar_vector& x, dvector& g, int _nvar, recaptype_vector& recaps, int nrec, coff_t<dvar3_array,dvar_matrix,dvar_vector,dvariable> &coff, coff_t<d3_array,dmatrix,dvector,double> &dfcoff, indexed_regional_fishery_record& irfr);
