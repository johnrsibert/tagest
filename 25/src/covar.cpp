//$Id: covar.cpp 2754 2011-01-02 20:57:07Z jsibert $
#include "precomp.h"
#include "fish_rec.h"
#include "intersav.h"
#include "factoral.h"
...
//dvector x(1, nvar);
  ...
  param.hess_comp(nvar_model, x, recaps, nrec, 2, irfr);
  ...
  param.sd_scale(scale, x);
  clogf << "\nScale:" << endl;
  for (int i = 1; i <= nvar_model; i++)
  {
    clogf << setw(3) << i << ": " << setw(20) << x_names(i) 
          << setw(15) << setprecision(5) << scale(i) << endl;
  }
  ...
  for (int i = 1; i <= nvar; i++)
  {
    clogf << setw(4) << i << ": " << setw(20) << x_names(i)
         << setscientific << setw(16) << setprecision(8) << x(i)
         << setw(16) << setprecision(8) << sqrt(stddev(i)) << endl;
  }
