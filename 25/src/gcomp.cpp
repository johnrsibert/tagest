//$Id: gcomp.cpp 2859 2011-10-03 18:52:55Z jsibert $
#include "precomp.h"
                ...
                const year_month& date, double& spawning_biomass)
{
  ...
  //j = move_corner(int(t.Lat.value()),sw_lat);
    ...
    int i1 = move_corner(m_ipar(56),sw_long);
    int j1 = move_corner(m_ipar(57),sw_lat);
    int i2 = move_corner(m_ipar(58),sw_long);
    int j2 = move_corner(m_ipar(59),sw_lat);
  ...
  else if (mode_flag == 5)
  {
    ...
    double c_bh = mfexp(d_bh*rcell); // replacement at equilibrium
    TTRACE(d_bh,c_bh)
  ...
  else if (mode_flag == 1)
  {
	...
	if (gridmap(i,j) > 0)
	{
	  g(i,j) = rmean*average_recruitment(j)*mfexp(rsig*randn(seed));
	}
	else
	  g(i,j) = 0.0;
      }
...
double normal_dev(const double x, const double mu, const double sigma)
{
  ...
  if (t < 80.0)
  {
    v = mfexp(-t*t)/sqrt(6.2831853 * sigma);
  }
  return(v);
}
