...
void setsaveinterflag::savebuf(intersavetype * isp)
{
  ...
  if (inbw != sizeof(double*))
  {
    ...
    fprintf(stderr, "Attempted to write %i bytes; actually wrote %i.\n",
      sizeof(double), inbw);
    exit(1);
  }
  isp->nb_write += inbw;
...
/*  if ( write(isp->savefile, isp->head, SAVE_BUF_SIZE*sizeof(double)) == -1) */
  ...
  if (unbw != save_buf_size*sizeof(double) )
  {
    ...
    fprintf(stderr, "Attempted to write %u bytes; actually wrote %u (%i).\n",
...
void setsaveinterflag::setsaveinter(int flag)
{
  ...
  if (flag == 1)
  {
    ...
    */
    ...
    if ( (isp = (intersavetype *)mem_malloc( sizeof(intersavetype) ) )== NULL)
    {
    ...
    if ( (isp->buf = (double*)mem_malloc(save_buf_size*sizeof(double)) )== NULL)
    {
