...
tpool::~tpool()
{
  //cerr << "in tpool::~tpool()" << endl;
  tpool_destroy(1);
  //cerr << "finished  tpool_destroy(1);" << endl;
}

int tpool::tpool_destroy(const int finish)
{
  //cerr  << "in int tpool::tpool_destroy("<<finish<<")" << endl;
...
void* tpool::tpool_thread(void *arg)
{
  ...
  for(;;) {
    ...
    {
      //printf("worker %u: I'm sleeping again, shutdown = %d\n", 
      //       pthread_self(),shutdown);

      if ((rtn = pthread_cond_wait(&(queue_not_empty), &(queue_lock))) != 0)
	 fprintf(stderr,"pthread_cond_wait %d",rtn), exit(1);

    }
    //sleep(5); 
 
    //printf("worker %u: I'm awake\n", pthread_self());
   ...
   // printf("worker %u: dequeing item %u, current_queue_size = %d\n",
   //         pthread_self(), my_workp->next,cur_queue_size);
    ...
    if (my_workp)
    {
      //printf("worker %u: executing item %u %u on %u\n",
      //      pthread_self(), my_workp, my_workp->routine,my_workp->arg);
...
void SEM_WAIT(sem_t *arg)        /* Ignore signal interruptions */
