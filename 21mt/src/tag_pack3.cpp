...
/*
void tag_packet::norm_walk(int& seed, year_month& date)
{
  double dx, dy;
  int m = pptr->get_m();
  ...
  if (k > 0)
  {
    ...
    double sigma = ug[k].sigma*tau;
    double edx = 30.0*ug[k].u*tau;
    double edy = 30.0*ug[k].v*tau;
    dx = sigma * randn(seed) + edx;
    dy = sigma * randn(seed) + edy;
    if (nsdd(k) < 1.0)
    {
      TTRACE(k,sigma)
      TTRACE(edx,edy)
  ...
  bound boundary;
  boundary = pptr->get_cell_type(i,j);
  x = set_pos(prevx, dx, xmin, xmax, 1, 1);
  y = set_pos(prevy, dy, ymin, ymax, 1, 1);
  add_step_stats(k, dx, dy);
}
*/
