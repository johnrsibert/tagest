...
par_t_reg::~par_t_reg()
{
  cerr << "Top of par_t_reg destructor usergrid.ptr = 0x" << hex << usergrid.ptr << dec << endl;
  if (usergrid.ptr != NULL)
  {
    delete usergrid.ptr;
    usergrid.ptr = NULL;
  }
  cerr << "Bottom of par_t_reg destructor usergrid.ptr = 0x" << hex << usergrid.ptr << dec << endl;
}
