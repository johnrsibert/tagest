...
int main(int argc, char ** argv)
{
  ...
  int max_threads = coff.get_max_threads();
  //int max_row_col = 2; //max(param.get_m(),param.get_n());
  //TTRACE(max_threads,max_row_col)
  adi_thread_pool = new tpool;
  adi_thread_pool->allocate(max_threads,max_threads,0);
 
  TRACE(sysconf(_SC_NPROCESSORS_CONF))
  TRACE(sysconf(_SC_NPROCESSORS_ONLN))
  ...
  if (param.m_ipar[13] == 2)
  {
    ...
    clogf << "\nrecaptures with renumbered cohorts"
          << "\n    fleet  date   c  i  j    n" << endl;
              //  SBPL  4 89/07   1 42 11    1
    clogf << recaps[1] << endl;
    for (int k = 2; k <= nrec;  k++)
    {
     
      if (recaps[k].date != recaps[k-1].date)
        clogf << endl;
      clogf << recaps[k] << endl;
    }
    */
