...
#include <sys\stat.h>
#endif
void y_curvature(const dmatrix& x, double& curv);
       ...
       indexed_regional_fishery_record& irfr)
{
 ...
 // u  v and sigma  initializtion was commented out earlier.  msa  2/12/2002 8:10PM
  ...
  const double uv_xweight = 1.0;
  const double uv_yweight = 0.5;
  const double sig_weight = 0.0;
  const double uv_mult = 1.0;
  const double sig_mult = 0.0;
  ...
  for (int cc = c1; cc <= c2; cc++)
  {
    ...
    for (date = local_start_date; date <= final_date; date++)
    {
      ...
      */
      ...
      double sig_penalty = 0.0;
      sigpen(sigma, sig_weight, sig_penalty);
      plike -= sig_penalty;
      total_sigma_penalty += sig_penalty;
  
      plike -= uv_mult*norm2(u);
      plike -= uv_mult*norm2(v);
      plike -= sig_mult*norm2(sigma);
    ...
    for (date = final_date; date >= local_start_date; date--)
    {
      ...
      //plike -= sig_mult*norm(sigma);
      df_sigma -= sig_mult*dfplike*2.0*sigma;

      //plike -= uv_mult*norm2(v);
      df_v -= uv_mult*dfplike*2.0*v;

      //plike -= uv_mult*norm2(u);
      df_u -= uv_mult*dfplike*2.0*u;
 
      double dfsig_penalty = 0.0;
      //plike -= sig_penalty;
      dfsig_penalty -= dfplike;
      //sigpen(sigma, sig_weight, sig_penalty);
      dfsigpen(sigma, df_sigma, sig_weight, dfsig_penalty);
      //double sig_penalty = 0.0;
      dfsig_penalty = 0.0;
      ...
      /*
      double dfcurv_pen = 0.0;
      //plike -= curv_pen;
      dfcurv_pen -= dfplike;
 
      //y_curvature(u, curv_pen);
      dfy_curvature(u, df_u, dfcurv_pen);
      //double curv_pen = 0.0;
      dfcurv_pen = 0.0;
      */
  ...
  TRACE(norm2(u))
  TRACE(norm2(v))
  TTRACE(norm2(sigma),sum(sigma))
  TTRACE(total_uv_penalty,total_sigma_penalty)
  time_t time_sec;
  time(&time_sec);
  clock_t time2 = clock();
