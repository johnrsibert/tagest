//$Id: fish_rc3.cpp 3206 2014-08-22 05:37:57Z jsibert $
/** \file
                   ...
                   year_month& pstart_date)
{
  ...
  for (int ef = 1; ef <= nfleet; ef ++)
  {
    TRACE(fleet_list(ef))
    const int pf = adstring_utils_index(pfleets,fleet_list(ef));
    TTRACE(ef,pf)
    TTRACE(fleet_list(ef),pfleets(pf))
    for (int ey = first_year; ey <= last_year; ey++)
    {
      for (int em = 1; em <= 12; em++)
      {
        year_month tdate(ey,em);
        const int pm = tdate - pstart_date;
        if ( (pm >= hs1) && (pm <= hs2) )
        {
          const int n1 = index(ef,ey,em);
          const int n2 = n1+nrec(ef,ey,em)-1;
          for (int n = n1; n <= n2; n++)
          {
            regional_fishery_record fr = fishery_records(n);
            const int ii = fr.i;
            const int jj = fr.j;
            effort(pm, pf, ii, jj) += fr.effort;
  ...
  // compute mean effort by fleet
  const int f1 = pfleets.indexmin();
  const int f2 = pfleets.indexmax();
  TTRACE(f1,f2)
  dvector mean_effort(f1,f2);
  mean_effort.initialize();
  dvector incidents(f1,f2);
  incidents.initialize();

  for (int m = hs1; m <= hs2; m ++)
  {
    //clogf << pm << " " << sum(effort(pm,0)) << endl;
    for (int f = f1; f <= f2; f++)
    {
      for (int i = effort(m,f).rowmin(); i <= effort(m,f).rowmax(); i++)
      {
        for (int j = effort(m,f,i).indexmin(); j <= effort(m,f,i).indexmax(); j++)
        {
          if (effort(m,f,i,j) > 0.0)
          { 
            incidents(f) ++;
            mean_effort(f) += effort(m,f,i,j);
          }
        }
      }
    }
  ...
  for (int f = f1; f <= f2; f++)
  {
    clogf << "Computing average effort for " << pfleets(f) << endl;
    if (incidents(f) > 0.0)
      mean_effort(f)/=incidents(f);
    else
      cerr << "  warning incidents for fleet " << f << " is <= 0" << endl;

    clogf << "  mean effort  for " << pfleets(f)
          << " is " << mean_effort(f)
          << ",  based on " << incidents(f) << " cases." << endl;
  }

  // normalize fishing effort to fleet means
  for (int m = hs1; m <= hs2; m ++)
  {
    for (int f = f1; f <= f2; f++)
    {
      if(mean_effort(f) > 0.0)
        effort(m,f) /= mean_effort(f);
    } 
  }
}
