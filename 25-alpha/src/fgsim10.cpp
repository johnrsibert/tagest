//$Id: fgsim10.cpp 3208 2014-08-30 06:54:21Z jsibert $
#include <time.h>
#include <prnstrem.h>
#include "par_t_reg.h"
#include "trace.h"
...
void write_xyz(const dmatrix& m, const par_t_reg<d3_array,dmatrix,dvector,double>* p, 
...
*/
template <>
void par_t_reg<d3_array,dmatrix,dvector,double>::simulate(JNIEnv* _env, jobject _jobj,
                         ...
                         const adstring& path, const adstring& file)
{
  ...
  if (graphics_on)
  {
    ...
    hpd.setColor(0,0,255,0);    // history(1), blue  total observed recaps
    hpd.setColor(0,255,0,1);    // history(2), green effortless recaps
    hpd.setColor(0,255,255,2);  // history(3), cyan  recaps with effort
    hpd.setColor(255,0,0,3);    // history(4), red   predicted recaps
  ...
  */
  ...
  d4_array effort_proxy;
  effort_proxy.allocate(0,duration);
  ...
  recaps.make_effort_proxy(effort_proxy, global_start_date, fleet_names);
  ...
  for (year_month date = start_date; date <= final_date; date++)
  {
    ...
    recaps.getreturns(Recaps1, date, 0);
    ...
    {
       cerr << "m_ipar(25) > 2 no longer supported" << endl;
       exit(1);
       elapsed_month =  date - global_start_date;

       fish_mort_comp(FishMort1, FishMort0, effort_proxy(elapsed_month), 
          date, d3aRelease(0), Recaps0, global_irfr.get_mean_effort() );//F=q*effort_proxy
    }
    total_mort_comp(FishMort1, FishMort0, dmTotMort,1);
    ...
    */
    ...
    if (graphics_on)
    {
      ...
      dmatrix dmTotalFishMort = slice_sum(FishMort1)+slice_sum(FishMort0);
      jni->drawSquareRegion(1.0-mfexp(-1.0*dmTotalFishMort), hr_plot);
      ...
      if(m_ipar(83)==1)
      {
          ...
          jni->drawSquareRegion(d3aRelease[0], td2_plot);
        ...
        if (seasonChanged)
        {
          ...
          jni->drawArrowsMovementRegion(get_seasonal_u(s),get_seasonal_v(s), fad_plot);
      ...
      if (m_ipar(53) > 0)
      {
        dmatrix TotalF1 = slice_sum(FishMort1);
        dmatrix TotalF0 = slice_sum(FishMort0);
      ...
      if (seasonChanged)
      {
        ...
        //jni->drawArrowsMovementRegion(get_seasonal_u(s), get_seasonal_v(s), mp_plot);       
      ...
      history(4) = sum(dmTotalPredCaps);             // 3, red
      jni->drawLinePlot(history, history_plot);
      //history(1) = sum(dmTotalObsCaps); // yes this seems out of step
      history(1) = sum(Recaps0(0))+sum(Recaps1(0));  // 0, blue
      history(2) = sum(Recaps0(0));                  // 1, green 
      history(3) = sum(Recaps1(0));                  // 2, cyan
    ...
    for (int cohort = 1; cohort <= max_cohorts; cohort++)
    {
      ...
      // solve the PDE
        ...
        deaths = elem_prod(d3aRelease[cohort],(1-mfexp(-dmTotMort)));
        //double fished = sum(deaths - d3aRelease[cohort]*(1-mfexp(-mort)));
        cum_deaths += deaths;
