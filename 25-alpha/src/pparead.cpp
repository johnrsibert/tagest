//$Id: pparead.cpp 3208 2014-08-30 06:54:21Z jsibert $
#include "par_t.h"
#include "bound.h"
#ifdef __NDPX__
  #include <iomanip.h>
...
//  cout << "Finished reading mort = " << mort << endl;
  ...
  global_start_date = get_tr_date(1);
  if (start_date != global_start_date)
  {
      start_date = global_start_date;
      cout << "Warning: Start date reset to date of first tag release: " 
           << start_date << endl;
      clogf << "Warning: Start date reset to date of first tag release: " 
           << start_date << endl;
  }

  final_date = get_tr_date(nrelease) + nmonth;
  duration = final_date - start_date;
  TTRACE(start_date,final_date)
  TRACE(duration)
  if (!recapture_dev)
  {
     recapture_dev.allocate(0,duration);
     cout << "Allocated recapture_dev." << endl;
     TTRACE(recapture_dev.indexmin(),recapture_dev.indexmax())
  }
  ...
  if (m_ipar(55) > 0)
  {
     ...
     if (!dfile)
     {
        cerr << "Error reading recapture_dev" << endl;
        ad_exit(1);
     }
     else
     {
        cout << "Read recapture_dev successfully." << endl;
        clogf << "Read recapture_dev successfully." << endl;
        clogf << recapture_dev << endl;
     }
  }

  /*  // won't work; par_t does know know recaps
  if (m_ipar[13] == 2) 
    last_cohort = collapse_cohorts(recaps,recaps.getTotalRecords());
  else
    last_cohort = nrelease;  
  */
   if (m_ipar(91) > 0)
   {
      dfile >> mean_tag_surv_prop >> tag_surv_prop_prior;
      last_cohort = m_ipar(91);
      tag_surv_prop.allocate(1,last_cohort);
      dfile >> tag_surv_prop;
      if (!dfile)
      {
         cerr << "Error reading tag_surv_prop" << endl;
         ad_exit(1);
      }
      else
      {
         cout << "Read tag_surv_prop successfully" << endl;
         clogf << "Read tag_surv_prop successfully" << endl;
         TRACE(tag_surv_prop)
      }
   }    
...
void par_t<D3_ARRAY,MATRIX,VECTOR,DOUBLE>::read_tagrelease(cifstream& istr)
{
  ...
  for (int k=1; k <= nrelease; k++)
  {
    ...
    if (!istr)
    {
      ...
      clogf  << "*   Tag release number " << k << ":" << endl;
      clogf  << "    cohort code number " << cohort << endl;
      clogf  << "    input longitude = " << longitude << " (" << tr[k].i << ")" << endl;
      clogf  << "    input latitute  = " << latitude  << " (" << tr[k].j << ")" << endl;
