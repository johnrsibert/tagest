//$Id: position.cpp 3125 2013-11-19 02:44:01Z jsibert $
#include <stdlib.h>
#include <ctype.h>
#include "modlarea.h"
...
// inverse translation
double Model_area::a_move_corner(const int i, const double dx, const int edge) const
{
...
{
  int i = move_corner(lng, deltax, int(sw_coord.long_value()));
  return(i);
}
...
{
  int j = move_corner(lat, deltay, int(sw_coord.lat_value()));
  return(j);
}
...
double Model_area::index_to_long(const int i) const
{
  double lng = a_move_corner(i, deltax, int(sw_coord.long_value()));
  return(lng);
}

double Model_area::index_to_lat(const int j) const
{
  double lat = a_move_corner(j, deltay, int(sw_coord.lat_value()));
  return(lat);
}  
