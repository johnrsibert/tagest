//$Id: par_t.cpp 3203 2014-08-15 03:17:55Z jsibert $
...
template <typename D3_ARRAY, typename MATRIX, typename VECTOR, typename DOUBLE>
void par_t<D3_ARRAY,MATRIX,VECTOR,DOUBLE>::deleteInstance() 
{
  if (instance != 0) {
    delete instance;
    instance = 0;
...
void par_t<D3_ARRAY,MATRIX,VECTOR,DOUBLE>::allocate()
{
  ...
  HERE
  q.allocate(fleet0,nfleet);
  report_rates.allocate(fleet0,nfleet);
  fleet_names.allocate(fleet0,nfleet);
  DomesticFleetKludgeFlag.allocate(fleet0,nfleet);
  nb_par.allocate(fleet0,nfleet);
  nb_par.initialize();
  minafm = 0.0;
  maxafm = 10.0;
  TRACE(num_afm)
  ...
  afm_j(t.afm_j)
{
  ...
  minmin_D = t.minmin_D;
  maxmin_D = t.maxmin_D;
  minlogslope = t.minlogslope;
  maxlogslope = t.maxlogslope;
  minloggamma = t.minloggamma;
  maxloggamma = t.maxloggamma;
...
tagreleasetype_vector::tagreleasetype_vector(const int n1,const int n2)
{
   v = (tagreleasetype_vrep *) malloc((size_t) sizeof(tagreleasetype_vrep));
   v->en1 = n1;
   v->en2 = n2;
   v->nsize = n2 - n1 + 1;
   v->refcnt = 1;
...
void tagreleasetype_vector::allocate(void)
{
   v->p = (tagreleasetype_ *) malloc((size_t) v->nsize * sizeof(tagreleasetype_));
   if (!v->p)
   {
      cerr << "New failed in tagreleasetype_vector allocation.\n";
      exit(1);
   }
}

tagreleasetype_vector::~tagreleasetype_vector()
{
   if (--v->refcnt == 0)
   {
      free(v->p);
...
tagreleasetype_ & tagreleasetype_vector::operator[](const int i)
{
   #ifndef OPT_LIB
   if ((i < v->en1) || (i > v->en2))
   {
      cerr << "subscript out of range in tagreleasetype_vector\n";
...
tagreleasetype_vector & tagreleasetype_vector::operator=(const tagreleasetype_vector & x)
{
   if ((x.v->en1 != v->en1) || (x.v->en2 != v->en2))
   {
      cerr << "Attempted assignment to tagreleasetype_vectors of unequal size.\n";
      exit(1);
   }
   x.v->refcnt++;
   v = x.v;
