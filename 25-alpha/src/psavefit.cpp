//$Id: psavefit.cpp 3207 2014-08-27 17:12:20Z jsibert $
#include "par_t.h"
...
void par_t<D3_ARRAY,MATRIX,VECTOR,DOUBLE>::savefit(ofstream& pf, const char* fullname)
{
  ...
  pf << "\n#   nmonth start_yr start_mo  nrelease \n"
     ...
     << endl;
  pf << "#\n#   nfleet (index starting at " << fleet0 << ")\n"
     << setw(8) << nfleet
     << "\n#" << endl;
...
//        << setw(8) << get_tr_cohort(k) //  cohort_index(k)
  ...
  if (m_ipar(55) > 0)
  {
     pf << "#" << "\n# recapture dev penalty = "
        << setw(25) << setprecision(17) << recapture_dev_pen << endl;
     pf << "# recapture_dev weight = " << recapture_dev_weight << endl;
     pf << "# recapture devs (" << recapture_dev.indexmin() 
        << " - " << recapture_dev.indexmax() << ")" << endl;
     for (int i =  recapture_dev.indexmin(); 
              i <= recapture_dev.indexmax(); i++)
     {
        pf  << setw(20) << setprecision(16)  << recapture_dev(i);
        if ( ((i+1) % 4) == 0)  // indexmin() is zero
           pf << endl;
     }
     if ( (recapture_dev.indexmax() % 4) != 0)
        pf << endl;
  }

  if ((m_ipar(91) > 0) || (m_ipar(92) > 0))
  {
     pf << "#" << "\n# tagging survivorship penalty = "
        << setw(25) << setprecision(17) << tag_surv_prop_pen << endl;
     pf << "# tagging survivorshp penalty weight = " << tag_surv_prop_weight << endl;
     pf << "# mean tagging survivorship" << endl;
     pf << mean_tag_surv_prop << endl;
     pf << "# tagging survivorship prior" << endl;
     pf << tag_surv_prop_prior << endl;
     pf << "# tagging survivorship by cohort (" << tag_surv_prop.indexmin()
        << " - " << tag_surv_prop.indexmax() << ")" << endl;
     if (!(!tag_surv_prop))
     {
        for (int i =  tag_surv_prop.indexmin(); 
                 i <= tag_surv_prop.indexmax(); i++)
        {
           pf << setw(20) << setprecision(16) << tag_surv_prop(i);
           if ( ((i) % 4) == 0)  // indexmin() is zero
              pf << endl;
        }
        if ( (tag_surv_prop.indexmax() % 4) != 0)
           pf << endl;
     }
  }
}
