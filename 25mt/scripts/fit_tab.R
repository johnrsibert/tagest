...
{
  print(root)
  pattern = paste(root,"\\.p[0-9][1-9,a-f]",sep="")
  print(pattern)
# files<-dir(".", pattern = paste(path,".*[0-9]$",sep=""))
...
# ret$fit<-fits
  ...
  nfit <- length(pars)
  print(nfit)
  cnames <- c("crit","steps","pool","uv","q","R","fleets","DO","seasons","m1","s11","s53","M","n","-log L","max |g|","nfe")
  print(cnames)
