...
like.prof.plot<-function(plt.file)
{
   ...
   attach(lpf)
   lwd = 3
   plot(prof.like$x,prof.like$Px,type='l',lwd=lwd,col="blue",
        xlab="x",ylab="P(x)")
   lines(norm.approx$x,norm.approx$Px,lwd=lwd)
...
{
   ota.file<-paste("fit",hex.fit,"_P1_ota.csv",sep="")
   print(ota.file)
   ...
   #pred <- pta$All[ndx]
   lwd < 3
   plot(edate[ndx],ota$with.effort[ndx], lwd=11, col="blue",lty="solid",ylim=yrange,
        xlab="Date",ylab="Recaptures",type='l')
   lines(edate[ndx],ota$with.effort[ndx], lwd=9, col="lightblue",lty="solid")
   lines(edate[ndx],ota$effortless[ndx], lwd=lwd-1, col="red",lty="solid")
   lines(edate[ndx],ota$total[ndx], lwd=lwd, col="purple",lty="solid")
   points(edate[ndx],pta$All[ndx],lwd=lwd, col="blue",cex=1.6,pch=19,type='b')
