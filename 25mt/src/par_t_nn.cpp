//$Id: par_t_nn.cpp 2754 2011-01-02 20:57:07Z jsibert $
#include "par_t_nn.h"
#include "trace.h"
...
{
    //spatial data
   depth_file_ext = ".zdt";   //water depth
   fads_file_ext = ".fad";  // number of FADs
   elev_file_ext =  ".zdf";  //elevation (bathymetry)
   dist_file_ext = ".dst";   //distance
   soi_index_file_ext = ".soi";   //monthly ENSO index

   //spatio-temporl data (read on the fly from binary files)
   wne_file_ext = ".wne";   //curl of the windstress, NE monsoon
   wsw_file_ext = ".wsw";  //curl of the windstress, SW monsoon
   wsc_file_ext = ".wsc";    //curl of the windstress (spatio_temporal)
   forage_file_ext = ".frg";  //forage field (Patrick Lehodey)
   sst_file_ext = ".sst";       //SST
   zonal_current_file_ext  = ".usc";   //zona current speed
   meridional_current_file_ext =  ".vsc"  ;  //meridional current speed
   habitat_file_ext = ".hab";          //habitat index
}
