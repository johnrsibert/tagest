//$Id: dfsigpen.cpp 2754 2011-01-02 20:57:07Z jsibert $
#include "par_t_reg.h"
...
void dfsigpen(par_t_reg<d3_array,dmatrix,dvector,double>& dfparam, par_t_reg<D3_ARRAY,MATRIX,VECTOR,DOUBLE>& param, year_month& date)
{
  ...
  if (param.m_ipar[41] > 0)
  {
    ...
    dvector& dfug = dfparam.get_usergridSigma(s);
    dvector&   ug =   param.get_usergridSigma(s);
 ...
 /* re- calculate differences in y direction */
   ...
   /* re-calculate differences in x direction */
    ...
    for (int k = ngrid; k >= 1; k--)
    {
      ...
      dfug[k] += dfsigma[k]*root_pi_1*0.5/sqrt(ug[k]);
      dfsigma[k] = 0.0;
    }
  }
}
template void dfsigpen(par_t_reg<d3_array,dmatrix,dvector,double>& dfparam, par_t_reg<d3_array,dmatrix,dvector,double>& param, year_month& date);
template<> void dfsigpen(par_t_reg<d3_array,dmatrix,dvector,double>& dfparam, par_t_reg<dvar3_array,dvar_matrix,dvar_vector,dvariable>& param, year_month& date) {}
