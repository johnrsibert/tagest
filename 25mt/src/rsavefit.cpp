//$Id: rsavefit.cpp 2948 2012-02-11 01:30:26Z eunjung $
#include "par_t_reg.h"
#include "trace.h"
...
//     << "# k             u                        v                sigma\n";
  ...
  if (m_ipar(83) == 1)
  {
     if (m_ipar(84) == 1)
     {
       ...
       pf  << "#\n# sustainable speed\n" 
           << setw(11) << sus_spd << endl;
     }

     if(m_ipar(85) == 1)
     {
       //pf << "#\n# region   active    min_D          slope           inflection\n";
       pf << "#\n# region   active    slope           inflection\n";
       for (int g = 1; g <= ngrid; g++)
       {
            pf<< setw(8) << g
              << setw(7) << fadmap_stratum_active_d(g)
              //<< setw(15) << setprecision(5) << min_D(g) << " " 
              << setw(12) << setprecision(5) << slope(g) << " "
