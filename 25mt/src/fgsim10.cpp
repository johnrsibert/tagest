//$Id: fgsim10.cpp 2983 2012-07-05 23:41:11Z jsibert $
#include <time.h>
#include <prnstrem.h>
#include "par_t_reg.h"
#include "trace.h"
                         ...
                         const adstring& path, const adstring& file)
{
  ...
  if (graphics_on)
  {
    ...
    */
    ...
    scm.region.setGridMap(gridmap);
    scm.legend.min = 1;
    scm.legend.max = 500000.0; //262144.0;
    scm.legend.type = 1;
    scm.legend.increment = 8.0;
    scm.legend.decimals = 0;
    jni->addSquareRegion(scm, dif_plot);
    ...
    if (m_ipar(83) == 1)
    {
      ...
      scm.region.title = "FAD density";
      scm.region.setGridMap(gridmap);
      scm.legend.min = 1;
      scm.legend.max = 120.0;
      scm.legend.type = 1;
      ...
      gx_plot = fm_layout(2);
      scm.region.title = "gx of FAD density";
      scm.region.setGridMap(gridmap);
      scm.legend.min = -1.5;
      scm.legend.max = 1.5;
      scm.legend.increment = 0.5;
      scm.legend.decimals = 1;
      scm.legend.type = 0;
      jni->addSquareRegion(scm,gx_plot);         
      
      gy_plot = fm_layout(3);
      scm.region.title = "gy of FAD density";
      scm.region.setGridMap(gridmap);
      scm.legend.min = -1.5;
      scm.legend.max = 1.5;
      scm.legend.increment = 0.5;
    ...
    if (m_ipar(53) > 0)
    {
      ...
      scm.region.title = "Harvest Ratio E > 0";
  ...
  for (year_month date = start_date; date <= final_date; date++)
  {
    ...
    */
    ...
    if (graphics_on)
    {
      ...
      if(m_ipar(83)==1)
      {
        jni->drawSquareRegion(gx_fadmap, gx_plot);
        jni->drawSquareRegion(gy_fadmap, gy_plot);
        
        jni->drawSquareRegion(fadmap, fad_plot);
               ...
               const adstring id, const int fit, const year_month date)
{
  ...
  ss << "xyz\\"
#endif
       << id << "-" << setw(2) << setfill('0') << hex << fit << dec
...
void write_bitmap(jnigraphics2* jni, const adstring id, const int fit, const year_month date, const int panel)
{
  ...
  jni->save_png(buf,640,480,panel);
}
...
void write_bitmap(jnigraphics2* jni, const adstring id, const int fit, const int panel)
{
  ...
  //jni->save_png(buf,640,480,panel);
  jni->save_png(buf,1280,960,panel);
}
