...
void bogus_dfadi(void)
{
  ...
  dvector rr = value(coff->_rr);
  dvector uu = value(coff->_uu);

  dvector dfrr(rr.indexmin(),rr.indexmax()); dfrr.initialize();
  dvector dfuu(uu.indexmin(),uu.indexmax()); dfuu.initialize();

  dvector work = value(coff->_work);
  dvector work1 = value(coff->_work1);
...
/*
void coff_t::dfadi(coff_t& coff, dmatrix& u, dmatrix& dfu, intersavetype *savefile)
{
...
void coff_t::dfadi(dmatrix &a_c, dmatrix &dfa, dmatrix &b_c, dmatrix &dfb, dmatrix &c_c, dmatrix &dfc, 
